...
def _draw_residence_pdf_content(p, width, height, commune_name="KEUR MASSAR", reference="RES-2026-0001", 
    ...
    # Timbre (en bas à gauche à la place du cadre)
    if os.path.exists(timbre_path):
        p.drawImage(ImageReader(timbre_path), 3*cm, 4*cm, width=4*cm, height=5*cm, mask='auto')
...
def generate():
    ...
    p.showPage()
    p.save()
    with open('certificat_residence_keur_massar_v6.pdf', 'wb') as f:
        f.write(buffer.getvalue())

generate()

import fitz
doc = fitz.open('certificat_residence_keur_massar_v6.pdf')
page = doc.load_page(0)
pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))
pix.save('C:/Users/senep/.gemini/antigravity-ide/brain/69704cf6-39b3-48ed-8c23-d8a83e781d23/certificat_residence_keur_massar_v6.png')
