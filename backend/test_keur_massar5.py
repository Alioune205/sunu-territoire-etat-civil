...
def _draw_residence_pdf_content(p, width, height, commune_name="KEUR MASSAR", reference="RES-2026-0001", 
    ...
    texte_complet = (
        ...
        f"que <b>{nom_complet}</b> né(e) le {date_naissance} à {lieu_naissance} et qu'il (elle) "
        f"réside à {adresse} au quartier {quartier} depuis {date_installation}."
    )
    
    para = Paragraph(texte_complet, style_center)
    # Marges: x=4cm, width=width-8cm.
    para_width = width - 8 * cm
    para_height = 5 * cm
    para.wrap(para_width, para_height)
    para.drawOn(p, 4 * cm, height - 10 * cm)
    ...
    if os.path.exists(timbre_path):
        p.drawImage(ImageReader(timbre_path), 8.5*cm, box_y + 2.5*cm, width=1.5*cm, height=2*cm, mask='auto')
    ...
    # TODO: Relier à la vraie logique de vérification on-chain plus tard
    ...
    p.drawImage(ImageReader(qr_buffer), qr_x, qr_y + 0.5*cm, width=2.5*cm, height=2.5*cm)
...
def generate():
    ...
    p.showPage()
    p.save()
    with open('certificat_residence_keur_massar_v5.pdf', 'wb') as f:
        f.write(buffer.getvalue())

generate()

import fitz
doc = fitz.open('certificat_residence_keur_massar_v5.pdf')
page = doc.load_page(0)
pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))
pix.save('C:/Users/senep/.gemini/antigravity-ide/brain/69704cf6-39b3-48ed-8c23-d8a83e781d23/certificat_residence_keur_massar_v5.png')
