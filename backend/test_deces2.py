...
def _draw_deces_pdf_content(p, width, height, dossier, officier, timbre_ref,
    ...
    style_normal = ParagraphStyle(name='Normal', fontName='Helvetica', fontSize=14, leading=22, alignment=TA_JUSTIFY)
    ...
    para_intro.wrap(width - 4 * cm, 15 * cm)
    para_intro.drawOn(p, 2 * cm, y_body - para_intro.height)
    y_body -= para_intro.height + 2.0 * cm
...
def generate():
    ...
    p.showPage()
    p.save()
    with open('certificat_deces_keur_massar_no_table.pdf', 'wb') as f:
        f.write(buffer.getvalue())

generate()

import fitz
doc = fitz.open('certificat_deces_keur_massar_no_table.pdf')
page = doc.load_page(0)
pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))
pix.save('C:/Users/senep/.gemini/antigravity-ide/brain/69704cf6-39b3-48ed-8c23-d8a83e781d23/certificat_deces_keur_massar_no_table.png')
